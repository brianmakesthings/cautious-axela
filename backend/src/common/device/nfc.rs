...
fn enable_bus() -> IOResult<Output> {
    Command::new("config-pin").arg("P9_19").arg("i2c").output()?;
			...
			for j in 0..data.len() {
				match index {
					...
					2 => {
						...
						else {return Err(std::io::Error::new(std::io::ErrorKind::Other, "Ack Response Err1").into());}
					},
					3 => {
						if data[j] == 0x00 {
							return Ok(());
							...
							return Err(std::io::Error::new(std::io::ErrorKind::Other, "Nack Error").into());
						}
						else if data[j] as u8 == 0x01 {
							return Err(std::io::Error::new(std::io::ErrorKind::Other, "App Error").into());
						}
					}
					_ => {
						index = 0;
		...
		for _ in 0..10 {
			...
			for j in 0..data.len() {
				match index {
					...
					2 => {
						if data[j] == 0xFF{index += 1; continue;}
						else {return Err(std::io::Error::new(std::io::ErrorKind::Other, "Ack Response Err2").into());}
					}
					3 => {
						match data[j] {
							0x01 =>{
